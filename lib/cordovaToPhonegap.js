...
exports.uploadProject = function uploadProject (projectPath, pgTokenArg, foldersToAdd) {
  let folderPath;
  let appId;
  let appData;

  // Cloning project to temporary path
  return fs.cloneProject(projectPath, foldersToAdd)
  .then(tempPath => {
    folderPath = tempPath
  ...
  .then(filePath => {
    const pgToken = pgTokenArg || process.env.PGToken
