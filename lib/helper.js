...
async function upload (mainFolder, extraFolders = [], token, options = {}) {
  ...
  const iosKeyName = ((options.keys || {}).android || {}).name || process.env.npm_package_config_keys_ios_name;
  const iosKeyPassword = ((options.keys || {}).android || {}).password || process.env.npm_package_config_keys_ios_password;

  if (androidKeyName || iosKeyName) {
    appOptions.keys = {};
    appOptions.keys = await pgInterface.findKeyIds(androidKeyName, iosKeyName );

    // TODO: Code to upload key: not fully tested
    // if (
