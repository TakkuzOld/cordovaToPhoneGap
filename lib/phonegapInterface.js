...
async function init (pgToken) {
  ...
  pgToken = pgToken || process.env.npm_package_config_pgToken || process.env.npm_config_pgToken

  if (!pgToken) { throw 'No PGToken defined.'; }

  pgApi = await pgAuth({ token: pgToken })
  pgGet = Q.denodeify(pgApi.get);
  pgPost = Q.denodeify(pgApi.post);
  pgPut = Q.denodeify(pgApi.put);
